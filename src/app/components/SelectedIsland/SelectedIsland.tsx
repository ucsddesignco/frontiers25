...
import type { CanvasState } from '@/app/stores/canvasStore';
...
const SelectedIsland = ({ selectedCard }: SelectedIslandProps) => {
  ...
  return (
    <div
      id="selected-island"
      className={`${selectedCard && showLightFog ? 'translate-y-0' : 'translate-y-[200%]'} fixed bottom-8 left-0 right-0 z-[2] mx-auto flex w-fit items-center justify-center gap-6 rounded-[32px] bg-red-500 py-2 pl-8 pr-[12px] transition-transform duration-300`}
    >
      <p>Selected</p>
      <GlassButton
        ...
        onClick={() => {
          ...
          const cardId = basePattern[selectedCardIndex].id;
          router.push(`/${cardId}`);
        }}
      >
        <svg
